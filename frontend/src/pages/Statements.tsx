...
export function StatementsPage() {
  ...
  const pollJob = useCallback((index: number, jobId: string) => {
    const interval = setInterval(async () => {
      try {
        const job = await statementService.getJob(jobId)
        if (job.status === 'FAILED') {
          clearInterval(interval)
          setUploads((prev) => prev.map((u, i) => (i === index ? { ...u, state: 'failed', error: job.error_message, jobStatus: job.status } : u)))
        } else if (['NEEDS_REVIEW', 'COMPLETED', 'DUPLICATE', 'OVERLAP_DETECTED'].includes(job.status)) {
          clearInterval(interval)
          setUploads((prev) => prev.map((u, i) => (i === index ? { ...u, state: 'completed', statementId: job.statement_id, jobStatus: job.status } : u)))
          queryClient.invalidateQueries({ queryKey: ['statements'] })
        } else {
          setUploads((prev) => prev.map((u, i) => (i === index ? { ...u, jobStatus: job.status } : u)))
        }
      } catch {
        clearInterval(interval)
      }
  ...
  const handleFiles = (files: FileList | null) => {
    ...
    Array.from(files).forEach((file) => {
      ...
      setUploads((prev) => [...prev, { file, state: 'uploading' }])
      uploadMutation.mutate(file, {
        onSuccess: (res) => {
          setUploads((prev) => prev.map((u, i) => (i === index ? { ...u, state: 'processing', jobId: res.job_id } : u)))
          pollJob(index, res.job_id)
        },
        onError: (err) => {
          setUploads((prev) => prev.map((u, i) => (i === index ? { ...u, state: 'failed', error: err.message } : u)))
        },
      })
    })
  }
              ...
              <tbody>
                {statements.map((s: any) => (
                  <tr key={s.id} className="border-b border-default last:border-0">
                    ...
                    <td className="px-4 py-2">{s.detected_bank}</td>
...
function UploadStatus({ state, error }: { state: UploadState; error?: string }) {
